...
Java 并发学习仓库：`docs/` 下 21 篇学习文档（00 全景地图 → 06 分布式），`src/test/java/com/sw/yang/concurrent/` 下按章节分包的验证代码。本文件随 git 同步，任何机器打开本仓库都会加载。
...
- 运行时若是 JDK 21，其 AQS 与文档讲解的经典版（JDK 8~20）实现不同（ConditionNode/位域状态，无 SIGNAL 常量），语义一致，对照 03-02 §7.11
