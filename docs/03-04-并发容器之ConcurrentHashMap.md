...
### 2.4 三层并发模型：并发度从哪来？（⭐ 总览）

```
并发度的来源是"划分"，不是锁：

  第 1 层  hash 划分（§2.1）
    不同 key 散到不同桶 → 不同桶之间没有共享状态 → 互不干扰
    期望并发度 = 桶数（JDK 7 只有 16 段，JDK 8 可达 2^30）
  第 2 层  桶内并发（§4）
    同一桶内的操作靠"锁头节点"互斥——锁的粒度必须对齐划分
  第 3 层  全局协同（§2.3 / §6 / §7）
    计数 baseCount/CounterCell、扩容 sizeCtl/transferIndex 都是跨桶状态
    → 全部用 CAS，不引入全局锁
```

```
三层各司其职（压缩哪个代价 / 守住哪个正确性）：
  划分      压缩竞争代价：O(全表) → O(单桶)   —— 消除"问题的影响半径"
  桶锁      守住分区内结构不变量：不丢节点、链表/树不混
  全局 CAS  守住协同原子性：计数不丢、扩容任务不重叠

反向佐证（§6）：扩容是唯一打破"划分独立"的操作（要碰所有桶）
  → 设计复杂度全部集中在那里（transferIndex / ForwardingNode / helpTransfer）
  → 而 JDK 8 连全局操作都拒绝上全局锁：宁可让 put 线程顺路帮忙
  → 同一哲学贯彻到底：能用分区解决的不上锁，能局部化的不全局化

注意措辞：并发问题没有被"避免"，而是被"局限化"
  hash 冲突时同一桶内照样退化（哈希攻击 → 单桶 O(n)）→ 树化兜底（§8）
  划分消除的是问题的影响半径，不是问题本身
```
...
### 6.7 为什么不上全局锁也能保证正确？（⭐ 正确性论证）

```
正确性 = 四个问题，四个机制：

  ① 每个桶恰好迁移一次（不重不漏）  → transferIndex CAS 认领区间
  ② 迁移过程没有中间态（桶永远完整）→ 空桶 CAS 换 fwd / 非空锁头 build-swap
  ③ 迁移期间 put/get 不丢数据       → ForwardingNode 分流 + 双检重试
  ④ 收尾唯一且原子（table 切换）    → sizeCtl 低 16 位计数判定最后线程
```

```
① 恰好一次：transferIndex 是唯一的"地盘指针"（初始 n，从尾往头走）
   认领 = CAS(transferIndex, nextIndex, nextBound) → 同一区间只有一个赢家
   → 区间互斥是数学保证，不是约定；输掉 CAS 的线程认领剩余区间
   不漏：认领区间内的桶（含空桶）必须全部处理完才能抢下一批
   → "表迁移完" = 每个区间都完整处理的局部事实累加

② 无中间态：桶迁移是"原子交换"
   空桶：一次 CAS 换成 ForwardingNode（单步原子）
   非空桶：锁头 + 双检 → 复制 ln/hn 两条链（不动旧链）→ setTabAt 换 fwd
   → 与树化同模式（§4.5 本地构建 + 单次发布）
   → 任何时刻桶只呈现"完整旧结构"或"ForwardingNode"两种状态
```

```
③ put 不丢数据的论证（与迁移者锁同一头节点 → 严格串行，两种顺序都对）：
   顺序 A：put 先得锁 → 尾插成功 → 迁移者遍历"当前"链（含新节点）一起搬走
   顺序 B：迁移者先得锁 → 换 fwd 后 put 双检失败 → 重走循环 → 见 MOVED
          → helpTransfer 去新表插入
   → 要么节点被搬走，要么自己被带到新表，数据都在
   get：读旧链 → 旧节点从不被修改 → 完整旧值；读 fwd → find 跳新表

④ 收尾唯一：每个参与者离开时恰好一次 CAS(sizeCtl, sc, sc-1)
   低 16 位回到基数 2（sc - 2 == 指纹 << 16）→ 自己是最后一个
   → 防御性复查所有桶都是 fwd → table = nextTab（单次 volatile 写）→ 新阈值
   判定唯一靠 CAS；切换原子靠 volatile
```

```
哲学：把"全局问题"降维成"局部问题"
   "表在不在扩容"   → 每个桶自己的 fwd 标记（状态本地化）
   "哪个桶归谁迁"   → transferIndex CAS 认领（任务分区）
   "还剩几个线程"   → sizeCtl 低 16 位（单变量计数）
   "迁移完了吗"     → 局部事实累加 + 最后线程复查

   → 任何参与者不用"问全局"，走到哪个桶看哪个桶的标记
   → 残存的全局状态只剩 sizeCtl 和 table 两个单变量，CAS/volatile 足够

对比全局锁方案（Hashtable 式）：锁全表 → 扩容期间 put 全部阻塞、吞吐归零
  CHM：扩容期间没有停摆时刻——put 要么照常进未迁移桶，要么顺路帮忙
```
