...
## 2. 为什么 wait 必须在 synchronized 块内调用
...
### 2.2 根本原因：等待与条件检查必须原子

```java
// 假设 wait 不需要锁，那么下面的逻辑是错的：
...
// → 这就是"先检查条件，再等待"之间的"竞态窗口"
// → 必须用锁把"检查条件 + wait"包成原子操作，notify 也必须在同一把锁内
```

### 2.3 正确理解：wait/notify 与锁的关系
...
2. **为什么 wait 必须放在 synchronized 里？**
   → 防止"检查条件"和"等待"之间出现竞态窗口。如果不加锁，检查完条件后、wait 之前可能丢失 notify，导致永久等待。
...
1. **为什么 wait() 必须在 synchronized 块内调用？不这样做会怎样？**
   ...
   为了把"条件检查 + wait"做成原子操作。否则在检查条件后、wait 之前可能丢失 notify，导致永久等待（竞态窗口）。不这样做直接调用会抛 IllegalMonitorStateException。
   </details>
